...
- Data shape is small today: `users/{uid}`, `users/{uid}/savedStrains/{strainId}`.
  See the rules file for the current shape; if you add a collection, add the
  rule.
...
experience behind a region-aware date-of-birth self-attestation. The age
gate is the **client-side popup** (localStorage + DOB attestation) — there
...
- AI callables (`compareStrains`, `recommendStrainsForConditions`,
  ...
  and the caller is already a signed-in user. There is **no
  `requireAgeVerified` check** in any callable. New AI callables do not
  need to add one.
- Verification records expire after 30 days
  (`AGE_VERIFICATION_TTL_MS`). The user re-prompts on the next session.
