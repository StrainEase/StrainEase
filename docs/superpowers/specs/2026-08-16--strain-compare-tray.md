...
| `StrainPoster.swift`'s `StrainRail` (defined inline at StrainPoster.swift:126–160; no separate file) | Picks up `compareStore` from `@Environment(CompareSelectionStore.self)` and forwards to each `StrainPoster`. |
