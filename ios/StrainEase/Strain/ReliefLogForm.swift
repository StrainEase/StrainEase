...
struct ReliefLogForm: View {
    ...
    private func save() async {
        ...
        await logs.add(
            ...
            relief: relief,
            note: note,
            // Map the existing `.any` chip to nil so the Firestore
            // document never carries a value web's PR-W1 enum
            // doesn't have. Web treats missing as unspecified.
            form: form == .any ? nil : form,
            doseMg: doseMg,
            timeOfDay: timeOfDay,
            onsetMinutes: onset,
...
struct ReliefHistoryList: View {
    ...
    private func sessionDetailRow(_ log: ReliefLog) -> some View {
        ...
        return sessionDetailBody(parts: parts, effects: effects)
    }

    @ViewBuilder
    private func sessionDetailBody(parts: [String], effects: [String]) -> some View {
