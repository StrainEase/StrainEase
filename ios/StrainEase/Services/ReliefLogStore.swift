...
// Session-journal enums. `ConsumeForm` already lives in
// `Models/StrainModels.swift` (with a `.any` case the existing
// DiscoverView prefs UI relies on); reuse that definition here so
// the module compiles without an ambiguous-type error.
//
// SessionTimeOfDay and SessionSideEffect are PR-W1-only, so they
// remain local to this file.

enum SessionTimeOfDay: String, CaseIterable, Identifiable, Hashable, Sendable {
    case morning
    case afternoon
    case evening
    case night
...
enum SessionSideEffect: String, CaseIterable, Identifiable, Hashable, Sendable {
    case anxiety
    case paranoia
