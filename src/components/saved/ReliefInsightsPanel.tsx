    ...
    <section className="rounded-2xl border border-border/70 bg-card px-5 py-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold tracking-tight">
            Your relief patterns
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Deterministic patterns from your existing logs — no AI
            interpretation.
          </p>
        </div>
        <TrendIcon
          className={cn(
            "size-5",
            analysis.trend === "declining" ? "text-amber-600" : "text-primary",
          )}
        />
      </div>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        <Metric
          ...
          value={analysis.averageRelief ? `${analysis.averageRelief}/5` : "—"}
        />
        <Metric
          icon={<Activity className="size-3.5" />}
          label="Logged experiences"
          value={String(analysis.totalEntries)}
        />
        <Metric
          icon={<TrendIcon className="size-3.5" />}
          label="Recent trend"
          value={analysis.trend.replace("-", " ")}
        />
      </div>
            ...
            {analysis.bestStrains.slice(0, 3).map((strain) => (
              ...
              >
                ...
                <span className="shrink-0 text-muted-foreground">
                  {strain.averageRelief}/5 ·{" "}
                  {Math.round(strain.justRightRate * 100)}% just right
                </span>
              </div>
            ))}
          </div>
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-background px-3 py-3">
      ...
      <p className="mt-1 text-base font-semibold capitalize tracking-tight">
        {value}
      </p>
    </div>
  );
