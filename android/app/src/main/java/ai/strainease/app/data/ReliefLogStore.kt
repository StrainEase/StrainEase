...
class ReliefLogStore(private val context: Context) {
    ...
    /**
     * Firestore listener hookup lives on the iOS / web side today;
     * the Android store is DataStore-backed (per `refresh()`), so this
     * is a no-op preserved to match the AuthBound lifecycle pattern
     * other stores use (`start(uid)` on sign-in, `stop()` on sign-out).
     */
    fun start(uid: String) { /* TODO: not used on Android today */ }

    /** Symmetric to [start] — see note above. */
    fun stop() { /* TODO: not used on Android today */ }
